...
まずは、自分以外のユーザーの一覧をトップページに表示してください。そこに名前やメールアドレスなどのユーザーの情報も表示しておきましょう。

次に、面接スケジュールをCRUD操作できるようにしていきます。

#### 面接スケジュールを表示・登録できるようにする
面接スケジュールを登録するためにはその情報をDBに保存する必要があります。
